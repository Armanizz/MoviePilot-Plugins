...
class MediaSyncDelStrm(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "mediasyncdelstrm_"
    # 加载顺序
    plugin_order = 9
    # 可使用的用户级别
    auth_level = 1
                    ...
                    'content': [
                        {
                            ...
                            'events': {
                                'click': {
                                    'api': 'plugin/MediaSyncDelStrm/delete_history',
                                    'method': 'get',
                                    'params': {
                                        'key': unique,
    ...
    def __get_transfer_his(self, media_type: str, media_name: str, media_path: str,
        ...
        if mtype == MediaType.MOVIE:
            ...
            # STRM 兼容：Emby Webhook 传入的 item_path 通常是 .strm 路径，
            # MoviePilot 转移记录的 dest 可能是真实媒体文件路径。
            # 因此电影删除时改为与整部电视剧一致，只按 tmdbid + mtype 匹配。
